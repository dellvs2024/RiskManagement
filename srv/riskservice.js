...
// Function handler: read-only
  this.on('calculateExposure', async (req) => {
    const { ID } = req.data;

    const risk = await SELECT.one.from('Risks').where({ ID });
    console.log(risk);
    if (!risk) return 0;

    // Example calculation: impact × probability
    return risk.impact * risk.prio;
  });

  // Action handler: modifies data
  this.on('changeOwner', async (req) => {
    const { ID, owner } = req.data;
    console.log(owner);
    const tx = req.tx;
    console.log(ID);
    const risk = await tx.run(
      UPDATE('Risks').set({ owner: owner }).where({ ID })
    );
